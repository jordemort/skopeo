...
git remote add upstream github:containers/skopeo.git
git remote add lsm5 github:lsm5/skopeo.git
git fetch --all
git checkout bionic
...
if [[ $SHORTCOMMIT == $CURRENT_SHORTCOMMIT ]]; then
    ...
    if [[ $VERSION == $CURRENT_VERSION ]]; then
       ...
       debchange --package "skopeo" -v "$VERSION-1~dev~ubuntu18.04~ppa1" -D bionic "bump to $VERSION, autobuilt $SHORTCOMMIT"
    ...
    echo "Pushing changes to lsm5/skopeo..."
    GIT_SSH_COMMAND="ssh -i $HOME/.ssh/id_rsa" git push -u lsm5 bionic -f

    echo "Sleeping 20 seconds to avoid Travis retrigger..."
    sleep 20
